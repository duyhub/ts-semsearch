...
- [x] **T4 (P1) — per-request execution trace on `/v1/semantic-search`** — structural summary (bm25Top, denseTop, constraintsEngaged/Applied/Relaxed, anchorGateFired, fallbackFired) **plus `steps[]` timing every pipeline stage** (parse → dense_retrieval → lexical_fusion → rank_signals → constraint_filter → anchor_gate → candidate_filter → explain_serialize) and `totalMs`. Rendered in the panel as a **per-step latency timeline** with bars + a stage strip. Threaded through `rank_scored` (zero cost when not requested); NOT on contract `/v1/search`. Warm total ~2 ms (G4 safe). Files: `pipeline.py`, `api.py`, `ui/admin.html`. ✅
- [x] **T5 (P1) — Tests** — `test_admin.py`, `test_pipeline_trace.py` (present/deterministic/absent-from-contract), `test_logging.py`. 118 passed. ✅
- [x] **T6 (P1) — per-request stage-timing log** — `logging_setup.py`, level via `SEMSEARCH_LOG_LEVEL`, one INFO record per request off the hot path listing every stage + its latency (`parse=0.06ms dense_retrieval=0.46ms …`), `propagate=False`. Files: `logging_setup.py`, `api.py`. ✅
